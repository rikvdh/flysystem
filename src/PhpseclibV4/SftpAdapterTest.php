...
class SftpAdapterTest extends FilesystemAdapterTestCase
{
    public static function setUpBeforeClass(): void
    {
        if (! class_exists(SFTP::class)) {
            self::markTestIncomplete("No phpseclib v4 installed");
        }
    }
    ...
    public function setupConnectionProvider(): void
    {
        ...
        $connection = self::connectionProvider()->provideConnection();
    ...
    public function it_can_proactively_close_a_connection(): void
    {
        ...
        self::assertTrue(self::$connectionProvider->connection->isConnected());

        $adapter->disconnect();

        self::assertFalse(self::$connectionProvider->connection->isConnected());
    }
    /**
     * @test
    ...
    {
        if (! self::$connectionProvider instanceof ConnectionProvider) {
            self::$connectionProvider = new StubSftpConnectionProvider('localhost', 'foo', 'pass', 2222);
        }

        return self::$connectionProvider;
    }
    ...
    {
        $provider = self::connectionProvider();
